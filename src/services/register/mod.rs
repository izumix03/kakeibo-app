...
use crate::models;
use crate::services::io::{read_data_or_create_new_data, write_to_json};
use crate::services::validate::InputValidator;
...
pub fn run(file_path: &str) {
    ...
    let mut data = read_data_or_create_new_data(file_path);
    data.push(item);
    write_to_json(&data, file_path);
}

fn input_register_type() -> u8 {
    println!("登録種別を入力して下さい(0: 収入、1:支出)");
    let mut register_type = String::new();

    io::stdin().read_line(&mut register_type).expect("登録種別の入力に失敗しました");
    let register_type: u8 = register_type.trim().parse().expect("登録種別は数値で入力して下さい");
...
fn input_name() -> String {
    ...
    io::stdin().read_line(&mut name).expect("品目名の入寮kに失敗しました");
    name.trim().to_string()
}

fn input_category_type(register_type: u8) -> u8 {
    println!("カテゴリーを入力して下さい");
    if register_type == 0 {
    ...
    io::stdin().read_line(&mut category_type).expect("品目名の入寮kに失敗しました");
    let category_type = category_type.trim().parse().expect("カテゴリは数値で入力して下さい");
...
fn input_price() -> u32 {
    ...
    price.trim().parse().expect("金額は数値で入力して下さい")
}
...
fn input_date() -> NaiveDate {
    ...
    NaiveDate::from_str(&date).expect("日付はyyyy-mm-ddの形式で入力して下さい")
}
